...
void main() {
  ...
  gl_FragColor += vec4(min(uDeepPurple, 1.), -1., .5, min(uOpacity, .8));
}
`;

// シェーダーマテリアルの型定義
export type CubeMaterialType = THREE.ShaderMaterial & {
  uFrequency: number;
  uAmplitude: number;
  uDensity: number;
  uStrength: number;
...
// シェーダーマテリアルの定義
export const CubeMaterial = shaderMaterial(
  {
    uFrequency: 0,
    uAmplitude: 0,
    uDensity: 0,
...
declare module '@react-three/fiber' {
  interface ThreeElements {
    ...
    } & React.RefAttributes<CubeMaterialType>;
  }
}
